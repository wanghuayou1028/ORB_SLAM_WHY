...
/**
 * @brief 并行地计算基础矩阵和单应性矩阵，选取其中一个模型，恢复出最开始两帧之间的相对位姿以及点云
 */
                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    mvMatches12.clear(); // mvMatches12只记录Reference到Current匹配上的特征点对
    mvMatches12.reserve(mvKeys2.size());
    mvbMatched1.resize(mvKeys1.size()); // 记录Reference Frame的每个特征点在Current Frame是否有匹配的特征点
    
    //vMatches12是已知的匹配关系，组织特征点对存放在mvMatches12变量中
    ...
    for(int i=0; i<N; i++)
    {
        vAllIndices.push_back(i); // 这个向量中保存的是从0到Ｎ-1的N个数字
    }
    ...
    for(int it=0; it<mMaxIterations; it++) //对每一组点对
    {
        vAvailableIndices = vAllIndices;

        // Select a minimum set
        for(size_t j=0; j<8; j++) //对每一组点８个中的一个进行赋值
        {
            // 产生0到N-1的随机数
            int randi = DUtils::Random::RandomInt(0,vAvailableIndices.size()-1);
            int idx = vAvailableIndices[randi]; //随机数randi对应的地idx个特征点对
...
{
    // Number of putative matches假定存在的匹配
    ...
    cv::Mat T1, T2;
    // mvKeys1和mvKeys2分别指参考帧和当前帧上提取的特征点
...
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    ...
    // const int N = vbMatchesInliers.size();//这里应该是错了，应该改成const int N = mvMatches12.size();
...
cv::Mat Initializer::ComputeH21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2)
{
	...
	*/
    ...
    return vt.row(8).reshape(0, 3);  // reshape是将矩阵恢复为三行，通道数不改变
}
...
cv::Mat Initializer::ComputeF21(const vector<cv::Point2f> &vP1,const vector<cv::Point2f> &vP2)
{
    ...
    w.at<float>(2)=0; // 秩2约束，将第3个奇异值设为0，这里也可以对前两个奇异值进行调整
...
void Initializer::Triangulate(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &P1, const cv::Mat &P2, cv::Mat &x3D)
{
    ...
    x3D = vt.row(3).t();
    x3D = x3D.rowRange(0,3)/x3D.at<float>(3); // 三维点转化为齐次坐标
}
...
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)
{
    float meanX = 0;
    float meanY = 0;
    const int N = vKeys.size(); // 特征点的个数

    vNormalizedPoints.resize(N); // 归一化后的点数设置为Ｎ

    for(int i=0; i<N; i++) // 将所有特征点的x,y方向坐标分别相加
    {
        meanX += vKeys[i].pt.x;
        meanY += vKeys[i].pt.y;
    }

    //得到所有特征点在x,y两个方向上的平均坐标位置
    meanX = meanX/N;
    meanY = meanY/N;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        meanDevX += fabs(vNormalizedPoints[i].x); // 将所有与平均值的偏差绝对值进行累加
        meanDevY += fabs(vNormalizedPoints[i].y);
    }

    // 平均偏差
    meanDevX = meanDevX/N; // 将累加的偏差除以Ｎ,相当于是平均分到每一个坐标点的误差
    meanDevY = meanDevY/N;

    float sX = 1.0/meanDevX;
    float sY = 1.0/meanDevY;

    // 将x坐标和y坐标分别进行尺度缩放，使得x坐标和y坐标的一阶绝对矩分别为1，使得关键点的累计偏差为１
    for(int i=0; i<N; i++)
    {
